const { Connection } = require('@angular/http');
...
// Queries 
const create_db = `CREATE DATABASE dan; USE dan`
const first_query = `USE dan; CREATE TABLE patient (
...
db.query(first_query,function(err,result){
    if (err) {
        ...
        console.log('Patients Add Successfully')
    }
});
...
db.query(all_patients,function(err,results){
    if (err) {
        ...
        for (let patient of results) {
            var update_ga = `USE dan;UPDATE patient SET Ga="${GA(patient.LMP)}" WHERE FirstName="${patient.FirstName}"`
            var update_patient_age = `USE dan;UPDATE patient SET patient_age="${patientAge(patient.DOB)}" WHERE FirstName="${patient.FirstName}"`
            var update_pregnancy_age = `USE dan;UPDATE patient SET pregnancy_age="${daysToToday(patient.LMP)}" WHERE FirstName="${patient.FirstName}"`
